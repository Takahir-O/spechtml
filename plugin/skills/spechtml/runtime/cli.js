...
const outputPath = fileArgs[1] ?? 'dist/specdoc.html';

if (!inputPath) {
  const err = { ok: false, error: 'Usage: node runtime/cli.js [--json] <input.toon> [output.html]' };
  if (jsonMode) {
...
try {
  ...
  try {
    ...
    safeOutput = await assertSafeOutputRealpath(outputPath, { allowedExt: ['.html'] });
  } catch (e) {
  ...
  await writeFile(safeOutput, renderDocument(data), 'utf8');
  ...
  if (jsonMode) {
    ...
    console.error(`Error (${err.phase}): ${err.error}`);
  }
  process.exit(1);
}
